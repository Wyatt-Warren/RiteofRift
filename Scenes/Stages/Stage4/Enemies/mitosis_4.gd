...
@onready var splitTimer = get_node("SplitTimer")
...
func _on_shoot_timer_timeout():
	...
	if arena.bullets < arena.bulletMax && distance > shootDistance:
		...
		new_bullet.target = target_vec.rotated(deg_to_rad(25))
		
		new_bullet2.position.x = global_position.x
		new_bullet2.position.y = global_position.y
		new_bullet2.target = target_vec.rotated(deg_to_rad(-25))
...
func _on_split_timer_timeout():
	if arena.enemyLessThanMax():
		arena.splitMitosis(generation, global_position, xp-1)
		queue_free()
