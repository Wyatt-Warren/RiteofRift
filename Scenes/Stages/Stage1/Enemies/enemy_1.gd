...
func _on_shoot_timer_timeout():
	...
	if arena.bullets < arena.bulletMax && distance > shootDistance:
		...
		new_bullet.position.x = global_position.x + 15
		new_bullet.position.y = global_position.y
		new_bullet.target = target_vec
		
		new_bullet1.position.x = global_position.x - 15
		new_bullet1.position.y = global_position.y
		new_bullet1.target = target_vec
		
		new_bullet2.position.x = global_position.x
		new_bullet2.position.y = global_position.y + 15
		new_bullet2.target = target_vec
		
		new_bullet3.position.x = global_position.x
		new_bullet3.position.y = global_position.y - 15
